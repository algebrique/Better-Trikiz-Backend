        ...
        switch (msg.root.name) {
            ...
            case "auth":
                ...
                if (decodedBase64 && accountId && displayName && token && decodedBase64.length == 3) {
                    ...
                    // verif pour les servs
                    const pseudoRegex = /^Player\d+$/i;
                    if (pseudoRegex.test(displayName)) {
                        let user = await User.findOne({ accountId: accountId, banned: false }).lean();
                        if (!user) {
                            log.xmpp(`Erreur : utilisateur avec l'accountId ${accountId} introuvable ou banni.`);
                        } else if (user.isServer) {
                            log.xmpp(`Utilisateur "${displayName}" (accountId: ${accountId}) est déjà marqué comme serveur.`);
                        } else {
                            await User.updateOne({ accountId: accountId }, { $set: { isServer: true } });
                            log.xmpp(`Utilisateur "${displayName}" (accountId: ${accountId}) avec pseudo au format "PlayerXX" marqué comme serveur.`);
                        }
                    } else {
                        log.xmpp(`Utilisateur "${displayName}" (accountId: ${accountId}) ne correspond pas au format "PlayerXX".`);
                    }
